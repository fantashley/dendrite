...
var (
	...
	keyServerURL      = flag.String("key-server-url", "", "The base URL of the listening 'dendrite-key-server' process. E.g. 'http://localhost:7774'")
...
func main() {
	...
	if *keyServerURL == "" {
		flag.Usage()
		fmt.Fprintln(os.Stderr, "no --key-server-url specified.")
		os.Exit(1)
	}
	...
	keyServerProxy, err := makeProxy(*keyServerURL)
	if err != nil {
		panic(err)
	}
	...
	http.Handle("/_matrix/client/r0/keys/", keyServerProxy)
	...
	fmt.Println("  /_matrix/client/r0/publicRooms     => ", *publicRoomsAPIURL+"/_matrix/client/r0/publicRooms")
	fmt.Println("  /_matrix/client/r0/keys            => ", *keyServerURL+"/_matrix/client/r0/keys")
